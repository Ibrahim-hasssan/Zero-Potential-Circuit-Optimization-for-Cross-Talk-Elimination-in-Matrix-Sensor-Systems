...
origTxt = "C:\\Users\\aboal\\Desktop\\zpc_project\\final tasks\\02-zpc_R11with multisin\\multisin with res.txt"
...
for indx, eachFile in enumerate(listALL):
    ...
    if '' in eachFile:
        ...
        component_vout00, phase_vout00 = find_fourier_component_and_phase(section_v_out_1, 500)
        component_vout01, phase_vout01 = find_fourier_component_and_phase(section_v_out_1, 1000)
        component_vout02, phase_vout02 = find_fourier_component_and_phase(section_v_out_1, 2500)
        component_vout03, phase_vout03 = find_fourier_component_and_phase(section_v_out_1, 5000)
 ...
 # print(f"figures #: {indx} - file name: {eachFile} - f = 200 Hz | measured resistance: Rmn = {Rmn:.4f} Ohms | capacitor: Cmn = {Cmn*1e12:.6e} pF")
    ...
    # Frequencies and their corresponding Fourier magnitudes
    frequenciesfft = [500, 1000, 2500, 5000]
    componentsfft = [component_vout00, component_vout01, component_vout02, component_vout03]

    # Plot the signal (V(out_col1)) in time domain
    plt.figure(1)
    plt.subplot(2, 1, 1)  # First subplot for time-domain signal
    plt.plot(time_Axis, trace_Axis[1], 'b-', linewidth=2, label='V(out_col1)')
    ...
    plt.grid(True)
    plt.title('Signal Output: ' + eachFile, fontsize=14, color='black')

   
    # Plot the frequency componentsfft as a line plot instead of bar (optional)
    plt.subplot(2, 1, 2)  # Second subplot for frequency componentsfft
    plt.bar(frequenciesfft, componentsfft, width=50, color='orange', label='Fourier Magnitudes')
    plt.xlabel('Frequency (Hz)', fontsize=12, color='black')
    plt.ylabel('Magnitude', fontsize=12, color='black')
    plt.grid(True)

    # Adjust legend position: move slightly right and up from the center
    plt.legend(loc='upper center', bbox_to_anchor=(0.7, 1.0), shadow=True, fontsize=10)
